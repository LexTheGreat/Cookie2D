...
namespace Cookie2D.Graphics
{
    ...
    public class MapRenderer : Drawable
    {
        ...
        private void SetSize(Vector2f v)
        {
            ...
            var length = width*height*4*Layers;
        ...
        private bool SetCorner(Vector2f v)
        {
            var tile = GetTile(v);
            var dif = tile - offset;
            if (dif.X == 0 && dif.Y == 0) return false;
            offset = tile;

            if (Math.Abs(dif.X) > width/3 || Math.Abs(dif.Y) > height/3)
            {
        ...
        /// <summary>
        /// Do draw and print time info
        /// </summary>
        [Conditional("DEBUG")]
        public void DrawDebug(RenderTarget rt, RenderStates states)
        {
            ...
            SetSize(view.Size);
            var refresh = SetCorner(rt.MapPixelToCoords(new Vector2i()));
            ...
            of -= offset;
            watch.Stop();
            if (refresh) Console.WriteLine("Size: {2},{3} Refresh: {0} ms Draw: {1} ms Diff: {4},{5}", time1, watch.Elapsed.TotalMilliseconds, width, height, of.X, of.Y);
        }
        ...
        public void Draw(RenderTarget rt, RenderStates states)
        {
            ...
            var length = width * height * 4 * Layers;
            rt.Draw(vertices,0, (uint)length, PrimitiveType.Quads, states);
        }
